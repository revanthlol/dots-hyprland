...
case $ask in
  ...
  *) 
    printf "${STY_BLUE}"
    printf "${STY_BOLD}Proceed with installation?${STY_RST}\n"
    printf "${STY_BLUE}"
    printf "  y = Yes, ask before executing each command. (DEFAULT)\n"
    printf "  n = Yes, execute commands automatically without asking each time.\n"
    printf "  a = Abort.\n"
    read -p "===> [Y/n/a]: " p
    case $p in
      n) ask=false ;;
